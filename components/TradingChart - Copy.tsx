...
const TradingChart: React.FC<TradingChartProps> = ({ data, config, symbol, positions, drawings, onAddDrawing, onRemoveDrawing, onChartTypeChange, theme }) => {
  ...
  const updateChartData = useCallback(() => {
    ...
    try {
        ...
        // Prepare data for Lightweight Charts (Time must be in seconds)
        const mainData = sortedData.map(d => {
  ...
  useEffect(() => {
    ...
    config.activeIndicators.forEach(ind => {
        ...
        try {
            ...
            // Update Series Data
            if (ind.type === 'Bollinger') {
                seriesObj.upper.setData(calculated.upper);
                seriesObj.lower.setData(calculated.lower);
                seriesObj.average.setData(calculated.average);
            } else if (ind.type === 'MACD') {
                seriesObj.macd.setData(calculated.macd);
                seriesObj.signal.setData(calculated.signal);
                seriesObj.histogram.setData(calculated.histogram);
            } else {
                seriesObj.setData(calculated);
